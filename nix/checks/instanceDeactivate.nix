...
pkgs.testers.runNixOSTest {
  name = "Deactivate an instance of a secret";
    ...
    { config, pkgs, ... }:
    {
      ...
      services.secret-agent = {
        ...
        secrets.db-creds = {
          ...
          activate = "echo active > /etc/creds";
          deactivate = "echo inactive > /etc/creds";
        };
      };
  ...
  testScript = ''
    ...
    machine.succeed(f"secret-agent activate db-creds {id}")
    machine.succeed(f"secret-agent deactivate db-creds {id}")
